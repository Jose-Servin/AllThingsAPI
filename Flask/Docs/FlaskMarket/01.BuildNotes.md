...
Modal windows allow us to focus our user's attention to a certain piece of information. For this project, we will build several modal windows to confirm critical actions such as purchasing, reading more about an item and selling an item.

The strategy here is to build dynamic modal windows using Bootstrap by passing our `item` object to fill in item specific data.

In our `Market.html` there are two buttons that open our modal windows, here we make sure to include out modal windows html file by leveraging jinja syntax.
...
<tr>
   ...
   <td>${{ item.price }}</td>
   <td>
      {# The data-target is what defined what modal window to run and it's how we dynamically change each Modal window #}
...
In order to tie our buttons with their respective Modal window we use two properties, one is the `data-toggle` and the other is `data-target` and pass our object using the jinja syntax `{{}}`.

For example, our "More Info" button contains the following properties:

```html
<button class="btn btn-outline btn-info" data-toggle="modal" data-target="#Modal-MoreInfo-{{ item.id }}">More Info</button>
```

In our `templates/includes/items_modals.html` the more info modal window contains this:
...
So, the `data-target` is the `id` of the Modal window we want to render and we an dynamically pass our item to this html file even though it's located in a separate directory.

## Item Purchasing Part 2

### Confirm Purchase Modal Window

For our Confirm Purchase modal window, we follow the same steps we used to tie our buttons to our modal windows but this time, we add a form to verify that the user wants to purchase an item and leverage this form to pass to our backend what item was purchased. In our backend we will edit our `routes/market.py` and add logic that checks if a User can purchase an item.

Our Confirm Purchase modal window contains the following:

```html
<!-- Purchase Item Confirm Modal Window-->
<div class="modal fade" id="Modal-PurchaseConfirm-{{ item.id }}"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                {# We can access the purchase_form because it's passed as context in our market.market_page route #}
                <form method="POST">
                    ...
                    <br>
                    {# The backend will consume the input element info in our Market route #}
                    {# This is how our backend will know what item was purchased. #}
...
Again, our `forms/purchase_form.py` is only used to capture the `POST` method and pass specific item information to our backend. To view this item information from our form, we can use the `purchase_form.__dict__` method and debug as needed.

What we will see here is that our `purchase_form` contains a submit field that is being translated to an `input` element each time a user clicks the `Submit` button to confirm purchase.

To summarize, our purchase form has a submit field, we call this submit field in our Modal window by using jinja syntax, in the backend, each time the submit field is clicked an input element gets generated therefore we will use this input element to pass out item information to out backend.
...
With this in mind, we can now move on and create a custom input element in our `items_modals.html`
...
A key thing here is that out input element is `hidden`, the only purpose of this element is to pass item information to our backend. In other words, "this is just background information the backend will consume."

Specifically, this background information is going to come from our `request` object, NOT our `purchase_form`. By default, this request object has access to our forms which is why we can access our form input data by using:

```python
purchased_item_name = request.form.get("purchased_item")
```

`.get()` looks at the `name` attribute and returns the `value` of our form input element. So now, we know exactly what item the user is trying to purchase.

## Purchase Items Checks

The first check we perform is to verify the `current_user` can purchase the item. This `current_user` is made available by the `flask_login` package.
...
The `can_purchase` check is in our `models/user` as a method so all we have to do is call it in our `routes/market.py`. Specifically, this checks if the current user's budget is greater than the item's price.
...
If the `current_user` can purchase the item, we then proceed with setting the current user as the item owner. The logic that implements this is written in our `models/item.py` file.
...
Again, all of this is invoked in our `routes/market.py`
...
So to summarize, in our `User` model we added the logic to check if a user can purchase an item and in our `Item` model we added the logic to actually assign an item to a User, specifically the `current_user`. This is basically a level of abstraction we did where the `Model` class contains the implementation and our `Route` contains the invocation.

## Form Resubmission JavaScript Warning

As we build our Purchase Item functionality, we ran into a JavaScript warning message pop up each time we attempted to refresh the page - `Confirm Form Resubmission`?

In order to handle this error, we created separate logic for both `POST` and `GET` request methods.

If the user is refreshing, that is a `GET` method so we query all available items for sale and render out `market.html` page.

"Items available in the market" means items without an owner.
...
If the user is submitting, that is a `POST` method so we capture information about the item they are purchasing or selling, run logic checks and redirect them to the market page, in other words, keep them on the market page.

See code comments on `routes/market.py` for more info.

## Selling Items

Selling is similar to Purchasing an item.

In our `routes/market.py` we begin by modifying our `GET` method by adding to the context `owned_items` which are items that are owned by the current user.
...
With this information available, we go to our `templates/market.html` and pass this variable to our frontend.

```html
<div class="col-4">
<h2>Owned Items</h2>
<p>Click on an Itemt to sell back to the Market.</p>
<div class="row">
    {%for owned_item in owned_items%}
    {% include 'includes/owned_items_modals.html' %}
    {# make sure to always define your grid system #}
    <div class="col-md-6">
        <div style="margin-bottom: 5px" class="card text-center bg-dark">
            <div class="card-body">
                <h5 class="card-title">{{ owned_item.name }}</h5>
                {# data-target is what triggers our Modal window #}
...
Next, we configure a modal window `templates/includes/owned_items_modals.html`. Which is very similar to Purchasing Item Modal therefore we also create the necessary form, found in `forms/sell_item_form.py`.

The final step in this is to add our "Selling" logic to our `routes/market.py`
...
Again, here we see the abstraction calls made by adding the logic to our `Models` and we see how `request.form.get("sold_item")` is the generated input field from our `sell_item_form.py` captured in the `./templates/includes/owned_items_modals.html` frontend.
...
<form method="POST">
    ...
    <br>
    <h6 class="text-center">
        By clicking Sell, you will sell this item back to the Market.
    </h6>
    <br>
    {# The backend will consume the input element info in our Market route #}
    {# This is how our backend will know what item was sold. #}
