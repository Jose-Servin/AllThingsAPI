...
3. Writing validation tests in our Form definition and executing them via our Route.
...
Here we see how our `Models` are tied with our `Forms`. In our `Model/User` we specified that the `username` field must be `unique=True`; therefor in our `Form/register_form`, when we submit a new user with an already existing username, we fail the unique constraint check which results in the SQLAlchemy error. And to reiterate, the call to run these validations occur in our `Route/register`.
