...
We see that the first row is taller than the second but only because `.el--3` has a height of `150px` which means, any grid items in that row will have to also be 150px to accommodate for ths property definition. Otherwise, the height would only be the necessary height needed for this content.
